...
async fn main() {
    ...
    let lyon_bt = NewTopic::new("lyon-bt-v2", 10, TopicReplication::Fixed(2))
        .set("max.message.bytes", "1234");
    let west_tht = NewTopic::new("west-tht-v2", 30, TopicReplication::Fixed(2))
        .set("max.message.bytes", "1234");
    let nuc_001_ht = NewTopic::new("nuc-001-ht-v2", 20, TopicReplication::Fixed(2))
        .set("max.message.bytes", "1234");
    let power_request = NewTopic::new("power-request-v2", 1, TopicReplication::Fixed(3))
        .set("max.message.bytes", "1234");
